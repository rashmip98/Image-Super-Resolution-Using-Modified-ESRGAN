...
import wandb
wandb.init(project="ESRGAN - exp0013", entity="willcf")
...
def train_generator(train_dataloader, epoch) -> None:
    ...
    wandb.log({"G Loss": pixel_loss.item()})
...
def train_adversarial(train_dataloader, epoch) -> None:
    ...
    for index, (lr, hr) in enumerate(train_dataloader):
        ...
        adversarial_loss = adversarial_weight * (adversarial_criterion(sr_output - torch.mean(hr_output), real_label) + adversarial_criterion(hr_output - torch.mean(sr_output), fake_label))
        # Update the weights of the generator model.
        ...
        if (index + 1) % 10 == 0 or (index + 1) == batches:
            print(f"Train stage: adversarial "
                  ...
                  f"D(HR): {d_hr:.6f} D(SR1)/D(SR2): {d_sr1:.6f}/{d_sr2:.6f} Adv Loss: {adversarial_loss:.4f} Pixel Loss: {pixel_loss} Content loss: {content_loss}.")
    wandb.log({"D Loss": d_loss, "G Loss": g_loss, "D(HR)":d_hr, "Adversarial Loss": adversarial_loss, "Pixel Loss":pixel_loss, "Content Loss":content_loss})
...
def main() -> None:
    ...
    best_psnr_value = 0.0
    # Load the model weights with the best indicators in the previous of training.
